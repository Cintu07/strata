...
**not yet implemented.** the format, the alignment and the read planner are all
built for it. writing the async direct-io backend is m2, and it cannot be
compile-tested on the windows host, so it is deliberately absent rather than
